...
import firrtl.transforms.TopWiring.{TopWiringAnnotation, TopWiringOutputFilesAnnotation, TopWiringTransform}
...
/** Provides signals for the transform to wire to the top-level of hte module hierarchy.
  ...
  */
case class BridgeTopWiringAnnotation(target: ReferenceTarget, clock: ReferenceTarget)
    extends Annotation
    with FAMEAnnotation
  ...
  def toWiringAnnotation(prefix: String): TopWiringAnnotation =
    TopWiringAnnotation(target.pathlessTarget.toNamed, prefix)
}

/** Provides reference targets to the newly generated top-level IO and a generated output-clock port that output is
  * synchronous to.
  *
  * @param pathlessSource
  *   The original target passed in a [[BridgeTopWiringAnnotation]]
  *
  * @param absoluteSource
  *   An absolute reference target to the particular instance of that signal that drives the new output. NB: A single
  *   [[BridgeTopWiringAnnotation]] will generate as many output annotations as there are instances of the pathless
  *   source.
  *
  * @param topSink
  ...
  *
  * @param clockPort
  *   The new output clock port the topSink port to be referenced in FCCAs
  */
...
/** A utility transform used to implement features that are finely distributed through out the target, such as assertion
  * and printf synthesis. This transform preforms most of the circuit modifications and analysis to emit
  * BridgeIOAnnotations and FCCAs directly. For this pass to function correctly, the clock bridge must already be
  * extracted.
  *
  * For each BridgeTopWiringAnnotation, this transform: 1) Wires out every instance of that signal to a unique port in
  * the top-level module These will be referenced by Bridge FCCAs and will become simulation channels. 2) Determines the
  * source clock (these are now inputs on the top-level module) to which each that port is synchronous
  *
  * For each clock that is synchronous with at least one output port: 1) Loop that clock back to a new output port
  * (Bridge FCCAs will point at this clock)
  *
...
class BridgeTopWiring(val prefix: String) extends firrtl.Transform {
  ...
  case class TopWiringMapping(src: ComponentName, instPath: Seq[String]) {
    ...
    def absoluteSourceRT(childInstGraph: Map[String, Map[Instance, OfModule]]): ReferenceTarget = {
      instPath
        .foldLeft[IsModule](topMT)((target, instName) => {
    ...
    def getSourceSinkPair(iMaps: Map[String, Map[Instance, OfModule]]): (ReferenceTarget, ReferenceTarget) =
      absoluteSourceRT(iMaps) -> portRT()
  }

  def execute(state: CircuitState): CircuitState = {

    require(
      state.annotations.collect({ case t: TopWiringAnnotation => t }).isEmpty,
    ...
    def wiringAnnoOutputFunc(
      ...
      topLevelOutputs = mappings.unzip._1.map(inscrutable5Tuple =>
        TopWiringMapping(inscrutable5Tuple._1, inscrutable5Tuple._4.dropRight(1))
      )
      state
    }
    ...
    val instanceMaps       = new InstanceGraph(wiredState.circuit).getChildrenInstanceMap
      ...
      .toMap
    // localRTtoAbsRTs
    val localToAbsSource   = topLevelOutputs
      .groupBy(_.src)
      .map({ case (src, mappings) =>
    ...
    val clockSourceMap = loweredState.annotations
      .collect({ case ClockSourceAnnotation(qT, source) =>
        qT -> source
      })
      .toMap
    ...
    val updatedCircuit = c.copy(modules = c.modules.map({
      case m: Module if m.name == c.main =>
        m.copy(ports = m.ports ++ addedPorts, body = Block(m.body, addedConnects.toSeq: _*))
      case o                             => o
    }))
        ...
        clockSourceMap(absClockRT) match {
          case Some(clock) =>
            BridgeTopWiringOutputAnnotation(
              localRT,
              absSourceRT,
              absSrcToPort(absSourceRT),
              clock,
              src2sinkClockMap(clock),
            )
